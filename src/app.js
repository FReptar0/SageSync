...
/**
 * CARGA DIARIA / DELTA — copia paralela de syncInventory().
 * Es una copia FIEL de la carga inicial; lo ÚNICO que cambia es la fuente de datos:
 * usa sage.getMovedInventoryItems() ("lo que se movió durante el día") en lugar de
 * sage.getAllInventoryItems() (todo). Todo el resto —los 3 casos A/B/C, mapeo de
 * almacén y endpoints de Fracttal— trabaja idéntico.
 *
 * OJO (deuda técnica): esto DUPLICA el loop de syncInventory a propósito, para no
 * tocar la carga inicial (acuerdo con Santiago). Si algún día se cambia la lógica
 * de los 3 casos, hay que reflejarlo en AMBAS funciones (o refactorizar a un motor
 * compartido _runInventorySync(sageItems, opts) que ambas llamen).
 */
async function syncInventoryMoved(options = {}) {
    const { dryRun = false } = options;
    const tag = dryRun ? '[DRY-RUN] ' : '';
    ...
    try {
        ...
        if (dryRun) {
            logger.info('[DRY-RUN] Iniciando dry-run de sincronización DIARIA (delta) — NO se enviará ninguna escritura a Fracttal');
        } else {
            logger.info('Iniciando proceso de sincronización DIARIA (delta) de inventario...');
        }
        ...
        // Obtener SOLO los items movidos desde Sage300 (carga diaria/delta) — ÚNICA diferencia vs. carga inicial
        const sageItems = await sage.getMovedInventoryItems();
        logger.info(`${tag}Se obtuvieron ${sageItems.length} items movidos desde Sage300 tras aplicar filtros de familia`);
        ...
        logger.info('='.repeat(60));
        logger.info(dryRun ? '[DRY-RUN] RESUMEN DE LA SIMULACIÓN (DIARIA/delta):' : 'RESUMEN DE SINCRONIZACIÓN DIARIA (delta):');
        logger.info(`- Total items en Sage300 (tras filtros): ${summary.totalItems}`);
        logger.info(`- Items procesados: ${summary.processedItems}`);
        logger.info(`- Case A (existe + en almacén → ajustar stock): ${caseCounts.caseA}`);
        logger.info(`- Case B (existe sin almacén → asociar+ajustar): ${caseCounts.caseB}`);
        ...
        if (dryRun) {
            logger.info('[DRY-RUN] Simulación DIARIA (delta) completada — no se realizó ninguna escritura a Fracttal');
        } else {
            logger.info('Proceso de sincronización DIARIA (delta) de inventario completado exitosamente');
        }
        return summary;

    } catch (error) {
        logger.error(`${tag}Error en la sincronización DIARIA (delta) de inventario:`, error);
        throw error;
    }
}
...
module.exports = { syncInventory, syncInventoryMoved };
